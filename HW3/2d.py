...
sigma_hpf = 64 / (2 * np.sqrt(2 * np.log(2)))
...
shifted_hpf_final = shifted2 * hpf_mask
...
axs[0, 1].axis('off')
axs[1, 1].imshow(np.log10(1 + np.abs(shifted_lpf_final)), cmap='gray')
axs[1, 1].set_title('Spectrum of LPF Image')
axs[1, 1].axis('off')
...
axs[0, 1].set_title('Spectrum of Original Image')
axs[0, 1].axis('off')
axs[1, 1].imshow(np.log10(1 + np.abs(shifted_hpf_final)), cmap='gray')
axs[1, 1].set_title('Spectrum of HPF Image')
axs[1, 1].axis('off')
axs[1, 0].imshow(hpf, cmap='gray')
axs[1, 0].set_title(f'High-Pass Filtered Image\nThreshold: {hpf_threshold} dB')
axs[1, 0].axis('off')
plt.show()
